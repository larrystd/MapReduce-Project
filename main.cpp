...
void *red_proc(){
    ...
    for(iter=m_pmap->begin(); iter!=m_pmap->end();iter++)
    {
...
void * map_function_proc(void* proc) {
  ...
  **/
  ...
  MySHMMap :: iterator iter;
  offset_ptr<MySHMMap> m_pmap1 = segment.find<MySHMMap>("MySHMMapName").first;
  for(iter=m_pmap1->begin(); iter!=m_pmap1->end();iter++)
  {
      //std::cout << iter->first << " has " << iter->second << std::endl;
  }
